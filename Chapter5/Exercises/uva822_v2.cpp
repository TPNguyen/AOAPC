// http://blog.csdn.net/wcr1996/article/details/41172445
#include <stdio.h>
#include <string.h>
#include <vector>
#include <iostream>
...
int main() {
    ...
    while (scanf("%d", &N) == 1 && N) {
        ...
        int working[10] = {}, preReq[10] = {}, finReq[10] = {}; // this is wrong, preReq should be filled with values less than 0 b/c 0 is valid preReq value
        int totTime = -2, now;
        timeline.push(-1);
        while (!timeline.empty()) {
            ...
            for (int i = 0; i < M; i++) {
                ...
                {
                    printf("Finished: Personnel %d at time %d\n", i, now);
                    working[i] = 0;
                }
            }
                ...
                for (int j = 0; j < u.proc_list.size(); j++) {
                    ...
                    if (!scheduler[tid].empty()) {
                        ...
                        if (tmp.st <= now) {
                            ...
                            printf("Started: Personnel %d on topic %d at time %d\n", D[i].second, tid, now);
                            break;
                        }
                    }
                }
