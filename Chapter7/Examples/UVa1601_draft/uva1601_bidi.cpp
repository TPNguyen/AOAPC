...
int main()
{
    while (scanf("%d %d %d\n", &w, &h, &n) == 3 && w)
    {
        ...
        Qf.push(initial);
        Qb.push(goal);
        dist_f.insert({initial.Hasher(), 0});
        dist_b.insert({goal.Hasher(), 0});

        while (!Qf.empty() && !Qb.empty())
        {
            auto cur = Qf.front(); Qf.pop();
            auto cur_dist = dist_f[cur.Hasher()];
            if (dist_b.count(cur.Hasher())) // alreday in back queue
            {
                printf("%d\n", cur_dist+dist_b[cur.Hasher()]);
                break;
            }
            ...
            for (auto i = 0; G[p0][i] >= 0; ++i)
            {
                ...
                for (auto j = 0; G[p1][j] >= 0; ++j)
                {
                    ...
                    for (auto k = 0; G[p2][k] >= 0; ++k)
                    {
                        ...
                        {
                            bool inserted = false;
                            std::tie(std::ignore, inserted) = dist_f.insert({next.Hasher(), cur_dist+1});
                            if (inserted)
                                Qf.push(next);
                        }
                    }
            ...
            cur = Qb.front(); Qb.pop();
            cur_dist = dist_b[cur.Hasher()];
            if (dist_f.count(cur.Hasher())) // alreday in front queue
            {
                printf("%d\n", cur_dist+dist_f[cur.Hasher()]);
                break;
            }
            ...
            for (auto i = 0; G[p0][i] >= 0; ++i)
            {
                ...
                for (auto j = 0; G[p1][j] >= 0; ++j)
                {
                    ...
                    for (auto k = 0; G[p2][k] >= 0; ++k)
                    {
                        ...
                        {
                            bool inserted = false;
                            std::tie(std::ignore, inserted) = dist_b.insert({next.Hasher(), cur_dist+1});
                            if (inserted)
                                Qb.push(next);
                        }
                    }
